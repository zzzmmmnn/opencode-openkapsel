...
For a client-backed directory, inspect kapsel_mappings first. The client has its own platform and sandbox policy; use kapsel_client_task when a literal argv array is needed.
    ...
    request: (args, context) => withSession(context, async ({ state, save, http, authorize, attribution }) => {
      ...
      if (method === 'POST' && rpcMatch) {
        ...
        if (!spec || typeof spec.write !== 'boolean') {
          throw new Error(`mapping RPC operation lacks write metadata: ${rpcMatch[2]}.${rpcMatch[3]}`);
        }
        rpcWrite = spec.write;
