...
export const PROMPT = `You operate a REMOTE OpenKapsel workspace. All file paths and shell commands refer to that server.
...
Use kapsel_shell_exec for remote execution and kapsel_task_output to poll results. Local tools and delegation are unavailable.
For a client-backed directory, inspect kapsel_mappings first. Client-side processes use kapsel_client_task, not server Shell; check the reported sandbox policy before starting one.
...
export function createBridge({ stateRoot, transport = runHelper } = {}) {
  ...
  return {
    ...
    request: (args, context) => withSession(context, async ({ state, save, http, authorize, attribution }) => {
      ...
      const readPost = method === 'POST' && /^\/?fs\/(read_many|manifest)(?:\?|$)/.test(endpoint);
