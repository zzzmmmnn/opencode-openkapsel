...
export function runHelper(payload, { cwd, signal, timeout = 130_000, maxBytes = 4_000_000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(process.platform === 'win32' ? 'python' : 'python3',
      ['-I', '-B', '-X', 'utf8', bootstrap], { cwd, env, shell: false, windowsHide: true, stdio: ['pipe', 'pipe', 'pipe'] });
    const chunks = [];
    let size = 0;
    let failure;
