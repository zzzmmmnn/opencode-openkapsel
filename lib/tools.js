...
const z = tool.schema;
const topics = ['overview', 'api-basics', 'endpoint-index', 'files', 'shell', 'context', 'memory', 'sharing', 'schedules', 'web-and-apps', 'mappings', 'data-rpc'];
const contextFields = {
...
export function createTools(options) {
  ...
  return {
    ...
    kapsel_archive: define('Browse or read a bounded member from a ZIP/tar archive without extracting it. Mapped archives use the client Archive RPC plugin.', {
      action: z.enum(['list', 'read']), path,
      inner_path: z.string().optional(), member: z.string().optional(),
      offset: z.number().int().nonnegative().optional(), limit: z.number().int().positive().optional(),
      encoding: z.string().optional(),
    }, ({ action, path: archivePath, inner_path, member, offset, limit, encoding }, ctx) => {
      if (action === 'read' && !member) throw new Error('member is required for archive read');
      return request('GET', `archive/${action}`, undefined, {}, ctx,
        action === 'list'
          ? { path: archivePath, inner_path, offset, limit }
          : { path: archivePath, member, offset, limit, encoding });
    }),
    kapsel_rpc: define('Invoke one dynamic mapping RPC operation. Inspect kapsel_mappings operation_specs first: execution=sync returns directly; execution=task returns a persistent client task_id that survives provider reconnects—poll with kapsel_task_output and do not replay a write RPC after transport uncertainty. write=true requires OpenCode write approval, Plan/Context, and a writable mapping. Future plugin families require no new OpenCode tool. No server/FUSE fallback.', {
      mapping_id: z.string(), family: z.string(), operation: z.string(),
      args: z.record(z.string(), z.unknown()).optional(),
      timeout_seconds: z.number().positive().optional(),
      ...contextFields,
    }, ({ mapping_id, family, operation, args, timeout_seconds, ...context }, ctx) =>
      request('POST',
        `mappings/${encodeURIComponent(mapping_id)}/rpc/${encodeURIComponent(family)}/${encodeURIComponent(operation)}`,
    ...
    kapsel_client_task: define('List/start legacy Shell tasks on a client mapping, and inspect/interrupt/kill either legacy raw task ids or unified client.<mapping>.<task> ids returned by kapsel_rpc/kapsel_shell_exec. RPC tasks do not accept stdin. Check kapsel_mappings first.', {
      ...
      const unified = task_id.startsWith('client.');
      if (unified && !task_id.startsWith(`client.${mapping_id}.`)) {
        throw new Error('unified client task id does not belong to mapping_id');
      }
      if (unified) {
        const endpoint = `tasks/${encodeURIComponent(task_id)}`;
        if (action === 'status') {
          if (offset !== undefined) throw new Error('use kapsel_task_output for offset-based unified task output');
          return request('GET', endpoint, undefined, {}, ctx);
        }
        if (action === 'stdin') throw new Error('RPC/unified tasks do not accept stdin through kapsel_client_task');
