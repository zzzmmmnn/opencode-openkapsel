...
export function createTools(options) {
  ...
  return {
    ...
    kapsel_rpc: define('Invoke one dynamic mapping RPC operation. Inspect kapsel_mappings operation_specs first: write=false is a read; write=true requires OpenCode write approval, Plan/Context, and a writable mapping. Future plugin families require no new OpenCode tool. No server/FUSE fallback.', {
      mapping_id: z.string(), family: z.string(), operation: z.string(),
      args: z.record(z.string(), z.unknown()).optional(), ...contextFields,
    }, ({ mapping_id, family, operation, args, ...context }, ctx) =>
      request('POST',
