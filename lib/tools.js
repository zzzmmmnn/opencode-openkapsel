...
export function createTools(options) {
  ...
  return {
    ...
    kapsel_mappings: define('List client-backed directories, connection status, write access, and advertised execution/RPC capabilities. Check before using a mapped path, RPC plugin, or client task.', {},
    ...
    kapsel_mapping_rpc: define('Invoke one advertised read-only RPC plugin operation on a mapped client. No Plan or write approval; no server/FUSE fallback.', {
      mapping_id: z.string(), family: z.string(), operation: z.string(),
      args: z.record(z.string(), z.unknown()).optional(),
    }, ({ mapping_id, family, operation, args }, ctx) =>
      request('POST',
