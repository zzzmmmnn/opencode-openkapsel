...
Invoke the installed script by its actual path while keeping the working directory at the controlling project. By default the command writes `.openkapsel.env` in that current directory with mode `0600`; `init --env-file <path>` or `OPENKAPSEL_ENV_FILE=<path>` selects a non-default credential file, whose parent directory must already exist. The command returns only its path/action/mode, is idempotent for identical credentials, and requires `--force` to replace a different existing configuration. Exclude the credential file from version control and do not place it in or upload it to the controlled workspace. The bundled helpers first use explicit CLI values, then the selected or nearest `.openkapsel.env`, then the legacy process environment. The helpers do not Shell-source this file; they parse only the documented keys without expansion.
